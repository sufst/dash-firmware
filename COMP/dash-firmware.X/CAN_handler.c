...
bool SendReverseRequest(uint8_t reverse) {
    // Check if CAN bus is in normal state
    if (CAN1_IsBusOff() || !CAN1_IsTxErrorActive()) {
        printf("CAN bus error, transmission aborted\n");
        return false;
    }

    // Pack dashboard data into TX buffer (Big-endian, Motorola format)
    tx_buff[0] =  reverse;

    // Attempt to transmit
    CAN_TX_MSG_REQUEST_STATUS status = CAN1_Transmit(TXQ, &tx_msg);
    
    if (status == CAN_TX_MSG_REQUEST_SUCCESS) {
        printf("Dashboard data transmitted successfully\n");
        return true;
    } else {
